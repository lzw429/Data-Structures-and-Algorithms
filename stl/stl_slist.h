...
#include <concept_checks.h>//包含基本定义与迭代器定义
...
__slist_make_link(_Slist_node_base *__prev_node,
	...
	__new_node->_M_next = __prev_node->_M_next;//新结点的下一结点
	__prev_node->_M_next = __new_node;//新结点上一结点指向新结点
	return __new_node;
}

//全局函数：找出__node的前面一个结点。由于是单向链表，需要从头查找。
//若找不到返回最后一个结点的next，即NULL.
inline _Slist_node_base *
...
__slist_previous(const _Slist_node_base *__head,
	const _Slist_node_base *__node) {//遍历循环的链表，找到__node前一个结点
	while (__head && __head->_M_next != __node)//此处的const表示指针指向的内容不可改变，并非指针本身不可修改
		__head = __head->_M_next;
	return __head;
}

//把(before_first, before_last]从原链表上剪切到到__pos后面。
inline void __slist_splice_after(_Slist_node_base *__pos,
	_Slist_node_base *__before_first,
	_Slist_node_base *__before_last) {
	if (__pos != __before_first && __pos != __before_last) {
		_Slist_node_base *__first = __before_first->_M_next;//不取before_first，而从before_first后一个开始，因为要考虑原链表
		_Slist_node_base *__after = __pos->_M_next;
		__before_first->_M_next = __before_last->_M_next;//从原链表剪切
...
__slist_splice_after(_Slist_node_base *__pos, _Slist_node_base *__head) {
	_Slist_node_base *__before_last = __slist_previous(__head, 0);//__before_last指向__head这条链表的最后一个结点
	if (__before_last != __head) {//如果__head链表非空
		_Slist_node_base *__after = __pos->_M_next;
		__pos->_M_next = __head->_M_next;
		__head->_M_next = 0;//__head失去作用
		__before_last->_M_next = __after;
	}
}
...
inline size_t __slist_size(_Slist_node_base *__node) {//统计结点个数
	size_t __result = 0;
...
//单向链表结点的结构。继承了_slist_node_base，其中有next指针，基类指针可指向派生类对象
template<class _Tp>
...
struct _Slist_iterator_base {
	...
	void _M_incr() { _M_node = _M_node->_M_next; }//迭代器向后移一位

	bool operator==(const _Slist_iterator_base &__x) const {//相等运算符==
		return _M_node == __x._M_node;//判断迭代器是否相等，即判断指向的结点是否相同
	}

	bool operator!=(const _Slist_iterator_base &__x) const {//不等运算符!=
		return _M_node != __x._M_node;
	}
};

//单向链表迭代器的派生类。派生类没有再定义数据成员，仅实现了数个成员函数。
...
struct _Slist_iterator : public _Slist_iterator_base {
	...
	reference operator*() const { return ((_Node *)_M_node)->_M_data; }//指针运算符*

#ifndef __SGI_STL_NO_ARROW_OPERATOR

	pointer operator->() const { return &(operator*()); }//成员运算符->
...
protected:
	typename _Alloc_traits<_Slist_node<_Tp>, _Allocator>::allocator_type
		_M_node_allocator;
	_Slist_node_base _M_head;//头结点，注意它不是指针
};
